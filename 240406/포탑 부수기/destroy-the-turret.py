...
def attacker(): # 공격자 선정 함수
    ...
    for y in range(N):
        for x in range(M):
            # 공격력이 가장 낮은.
            if tmp[0] > game[y][x] and game[y][x]:
                if [y, x] == tmp[1:]:
                    isMe = True
                tmp = [game[y][x], y, x]
            # 자신이 선택된걸 제외
            elif tmp[0] == game[y][x] and not isMe and not game[y][x]:
                if tmp[1:] == [y, x]:
                    tmp[0] = game[y][x]
                    isMe = True
                elif sum(tmp[1:]) < (y+x):
                    tmp = [game[y][x], y, x]
                # 열이 높은
                elif sum(tmp[1:]) == (y+x) and tmp[2] < x:
                    tmp = [game[y][x], y, x]
    return tmp
